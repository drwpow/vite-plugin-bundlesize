...
export default function analyze({
  ...
  for (const entry of Object.values(bundlemeta.chunks)) {
    ...
    if (!globMatch) {
      console.log(
        `${FG_YELLOW_220}! Entry "${entry.id}" not matched by any patterns in bundlesize.limits. Using default size of 150 kB.`,
      );
    }
  ...
  if (passed.every((status) => status === true)) {
    ...
    console.log(
      `${FG_RED_197}The following chunks exceeded the size limit:${RESET}`,
    );
  }
  ...
  console.log(""); // newline
  for (let i = 0; i < chunks.length; i++) {
    const nodeModulesTotal = Object.values(chunks[i].contents).reduce(
      (size, c) => size + (c.packageName ? c.size : 0),
      0,
    );
    ...
    if (!passed[i] || config.stats === "all") {
      ...
      for (const c of Object.values(chunks[i].contents)) {
        if (c.packageName) {
          if (!(c.packageName in nodeModulesSizes)) {
            nodeModulesSizes[c.packageName] = 0;
          }
      ...
      console.log(
        `${passed[i] ? FG_GREEN_79 : FG_RED_197}├─${RESET} Biggest modules`,
      );
      console.log(ls); // newline
      let remainingModuleSize = nodeModulesTotal;
      for (const [packageName, size] of config.stats === "all"
        ? sortedNodeModules
        : sortedNodeModules.slice(0, 25)) {
        ...
        console.log(
          `${ls}${packageName}${padLeft(`${displaySize} ${DIM}${displayPerc}${RESET}`, maxLength - packageName.length)}${RESET}`,
        );
        const bar = Math.round(maxLength * perc);
        console.log(
          `${ls}${FG_RED_197}${"=".repeat(bar)}${RESET}${"-".repeat(maxLength - bar)}${RESET}`,
        );
      ...
      if (config.stats === "summary" && sortedNodeModules.length > 25) {
        ...
        console.log(
          `${ls}${DIM}… and ${remaining} other${remaining !== 1 ? "s" : ""} (${nf.format(remainingModuleSize / 1000)} kB)${RESET}`,
        );
      }
    }
