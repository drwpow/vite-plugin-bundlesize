...
async function main() {
  ...
  const plugin = viteConfig.plugins.find(
    ({ name }) => name === "vite-plugin-bundlesize",
  );
  if (!plugin) {
    const configFile = viteConfig.configFile
      ? path.relative(process.cwd(), viteConfig.configFile)
      : "vite.config.ts";
...
${FG_GREEN_79}+     bundlesize()${RESET}`);
  ...
  if (!fs.existsSync(config.outputFile)) {
    const outputFile = config.outputFile.href.replace(
      new URL(`file://${process.cwd()}/`).href,
      "",
    );
    console.error(
      `${FG_RED_197}✘ vite-plugin-bundlesize: ${outputFile} not found. Run \`vite build\` to create it.${RESET}`,
    );
    process.exit(1);
  }
